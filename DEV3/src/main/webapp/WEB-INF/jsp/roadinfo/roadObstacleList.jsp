...
<c:set var="bundleName"
	value="bundles.lang_${not empty authInfo.changedCdNa ? authInfo.changedCdNa : authInfo.cdNa}" />
...
<div class="contentsWrap">
	...
	<ul class="search_box" style="min-width: 1390px;">
		<li><input type="text" value="" name="" id="inputVideoName"
			class="input1"
			placeholder="<fmt:message key="VIDEO_NAME" bundle="${bundle}"/>">
			<span class="stl"><fmt:message key="PERIOD" bundle="${bundle}" /><span
				class="remark2"></span></span> <span class="date"> <input
				type="text" value="${fromDt}" name="start" id="fromDt"
				class="input2" readonly> ~ <input type="text"
				value="${toDt}" name="end" id="toDt" class="input2" readonly>
				<button class="search_calender" id="search_calender"></button>
		</span>
			<button class="btn_search">
				<fmt:message key="SEARCH" bundle="${bundle}" />
			</button></li>
		<button class="btn_subPrimary btn_mapinfo" id="btn_mapinfo"
			onclick='showMap()'>
			<fmt:message key="MAP_INFO" bundle="${bundle}" />
		</button>
	...
	<!-- 도로 장애물 탐지 상세정보 -->
	<div class="contents_box roadinfo roadDetetionInfo"
		id="mapDetetionInfo" style="display: none;">
		<div class="btnAreaTop" style="display: none">
			<button class="btn_re-search" id="btn_re-search" onclick='btnClick()'>
				<fmt:message key="RESULT_IN_SEARCH" bundle="${bundle}" />
			</button>
			<button class="btn_bgPrimary btn_obstacleList" id="btn_obstacleList"
				onclick='hideMap()'>
				<fmt:message key="OBS_LIST_INFO" bundle="${bundle}" />
			</button>
		</div>
		<div class="contents mainMap" style="height: calc(100% -64px);">
			<div class="mapWrap">
				...
				<ul class="search_box level_list"
					style="min-width: 1170px; margin-top: 0px; display: none;">
					<li><span class="selectBox resp bottom" id="msgdivCd_span">
							<button class="label" id="level1" data-code="" data-lat=""
								data-lng="">Level1</button>
							<ul class="optionList" id="level1_ul"></ul>
					</span> <span class="selectBox resp bottom" id="msgdivCd_span">
							<button class="label" id="level2" data-code="" data-lat=""
								data-lng="">Level2</button>
							<ul class="optionList" id="level2_ul"></ul>
					</span> <span class="selectBox resp bottom" id="msgdivCd_span">
							<button class="label" id="level3" data-code="" data-lat=""
								data-lng="">Level3</button>
							<ul class="optionList" id="level3_ul"></ul>
					</span> <span class="search" style="display: none;"> <input
							type="text" value="" name="deviceNm" id="deviceNm" class="input1"
							placeholder="디바이스명">
					</span> <span class="stl"> <fmt:message key="PERIOD"
								bundle="${bundle}" /> <span class="remark2"></span>
					</span> <span class="date"> <input type="text" value="${fromDt}"
							name="start" id="fromDt2" class="input2" readonly> ~ <input
							type="text" value="${toDt}" name="end" id="toDt2" class="input2"
							readonly>
							<button class="search_calender" id="search_calender"></button>
					</span>
						<button class="btn_search">
				...
				<!-- 결과내재검색패널 -->
				<div class="re-search-container" id="re-search-container"
					style="display: none; width: calc(100% - 400px);">
					<button type="button" id="btn_re-searchWrap"
						class="btn_re-searchWrap" onclick='btnClick()'></button>
					<div class="group">
						<dl class="">
							<dt>
								<fmt:message key="DETECTED_OBJ" bundle="${bundle}" />
							</dt>
							<dd>
								<div class="selectOpt">
									<c:forEach var="obslist" items="${codeListOb}"
										varStatus="status">
										<input type="checkbox" id="${obslist.etc1}" class="obslist"
											name="obslist" value="${obslist.etc1}" checked>
										<label for="${obslist.etc1}"> <c:choose>
						...
						<div class="submitarea">
							...
							<button class="btn_iconTXT btn_r btn_m btn_resetOpt"
								onclick="optionReset()">
								<fmt:message key="RESET_SELECT" bundle="${bundle}" />
							</button>
						</div>
					</div>
				</div>
						...
						<div class="pop_wrap" id="pop_riskPopImg" style="display: none;">
							<div class="pop_riskPopImg">
								<div class="photo_area">
									<img id="pop_img" src="/img/sample_highway03.jpg"
										alt="도로위험목록 이미지">
								</div>
							</div>
			...
			<div class="infoWrap on">
				...
				<div class="infoDetailWrap" style="display: none;">
					...
					<div class="infoContents">
						<p class="infoPhoto">
							<img id="detail_img" src="/img/sample_highway02.jpg" alt="대표이미지"
								onclick="originalimg()"> <span class="noPhoto"
						...
						<div class="infoDetail">
							<div class="itemvalue roadDeteil">
								<span><fmt:message key="ANIMALS" bundle="${bundle}" /><em
									id="animalsCnt">0</em></span> <span><fmt:message key="PERSON"
										bundle="${bundle}" /><em id="personCnt">0</em></span> <span><fmt:message
										key="CONSTRUCTION_SIGNS" bundle="${bundle}" /><em
									id="constructionSignsCnt">0</em></span> <span><fmt:message
										key="TRAFFIC_CONE" bundle="${bundle}" /><em
									id="trafficConeCnt">0</em></span> <span><fmt:message key="BOX"
										bundle="${bundle}" /><em id="boxCnt">0</em></span> <span><fmt:message
										key="STONE" bundle="${bundle}" /><em id="stoneCnt">0</em></span> <span><fmt:message
										key="GARBAGE_BAG" bundle="${bundle}" /><em id="garbageBagCnt">0</em></span>
								<span><fmt:message key="MANHOLE" bundle="${bundle}" /><em
									id="manholeCnt">0</em></span> <span><fmt:message key="POTHOLE"
										bundle="${bundle}" /><em id="potholeCnt">0</em></span> <span><fmt:message
										key="FILLED_POTHOLE" bundle="${bundle}" /><em
									id="filledPotholeCnt">0</em></span>
							</div>
							<dl class="">
								<dt>
					...
					<div class="btnWrap">
						<div class="btnArea">
							<button type="button" class="btn_iconTXT" id="btn_Back">
								<span class="material-symbols-outlined">list</span>
								<fmt:message key="TO_LIST" bundle="${bundle}" />
							</button>
						</div>
					</div>
				</div>
		...
		for (var x = 0 ; x < markerList.length ; x++ ) {
   			...
   			if (markerList[x] == i) {
				...
				infoList.push("<li><a class='infoListItem'>"
					+ "<div class='info'>"
					+ "<ul class='infoContents' style='cursor: pointer;' onClick=\"detail('" + id + "', 'N', 'Y')\">"
