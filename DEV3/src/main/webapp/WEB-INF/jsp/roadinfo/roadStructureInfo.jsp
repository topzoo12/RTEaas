...
<div class="contents_box item mainpage roadinfo">
	<p class="title ${fav}">${pageName.srnNm}도로시설물 위치정보</p>
		...
		for (var x = 0 ; x < markerList.length ; x++ ) {
   			...
   			if (markerList[x] == i) {
				...
				var t1 = L.marker([item.point.latitude, item.point.longitude], {
					...
					status : item.status,
					delineators : item.risk['count-of-delineators'],
...
function detail(id, clusterChk, listClicked){
	...
	var deviceNm, deviceId, addrPoLocality, dateFormat, lat, lng, level, status, potholes, vertical, horizontal, alligators, riskLvNm,highway,delineators;
	...
	markerCluster.eachLayer(function(layer) {
		if (layer.options.id === id) {
			...
			delineators = layer.options.delineators;
	...
	// 포트홀
	$("#Cntdelineators").text(delineators + " 개")
	/* // 수직균열
