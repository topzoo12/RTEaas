		...
		<ul class="search_box level_list" style="min-width: 1170px; margin-top:0px; width: calc(100% - 40px);">
	        <li>
	            ...
	            <span class="search" style="display: none;">
					...
					<button class="btn" onclick="ttt()">경로</button>
				...
				<li>
					<button class="btn_re-search" onclick='btnClick()'>결과 내 재검색</button>
				</li>
	    </ul>
	    <!-- 결과내재검색패널 -->
			<div class="re-search-container" id="re-search-container" style="display: none; width: calc(100% - 40px);">
                <button type="button" class="btn_re-searchWrap"></button>
				...
				<div class="group">
					<dl class="">
						...
						<dd>
							<div class="selectOpt">
								<input type="checkbox" id="riskAll" name="riskAll" checked><label for="riskAll">전체</label>
								<input type="checkbox" id="risk1" name="risk" checked><label for="risk1">1</label>
								<input type="checkbox" id="risk2" name="risk" checked><label for="risk2">2</label>
								<input type="checkbox" id="risk3" name="risk" checked><label for="risk3">3</label>
								<input type="checkbox" id="risk4" name="risk" checked><label for="risk4">4</label>
								<input type="checkbox" id="risk5" name="risk" checked><label for="risk5">5</label>
						</div>
						</dd>
					</dl>
				</div>
				<div class="group">
					<dl class="">
						...
						<dd>
							<div class="selectOpt">
								<input type="checkbox" id="allCrack" name="allCrack" checked><label for="allCrack">전체</label>
								<input type="checkbox" id="pothole" name="crack" checked><label for="pothole">포트홀</label>
								<input type="checkbox" id="horizontal" name="crack" checked><label for="horizontal">수평균열</label>
								<input type="checkbox" id="vertical" name="crack" checked><label for="vertical">수직균열</label>
								<input type="checkbox" id="alligators" name="crack" checked><label for="alligators">피로균열</label>
						</div>
						</dd>
					</dl>
				</div>
				<div class="group">
					<dl class="">
						...
						<dd>
							<div class="selectOpt">
								<input type="checkbox" id="statusAll" name="statusAll" checked><label for="statusAll">전체</label>
				...
				<div class="group">
					<dl class="">
						...
						<dd>
							<div class="selectOpt">
								<input type="radio" id="on" name="markerPower" checked onclick="markerOn()"><label for="on">ON</label>
								<input type="radio" id="off" name="markerPower"onclick="markerOff()"><label for="off">OFF</label>
							</div>
						</dd>
					</dl>
					<div class="submitarea">
						<button class="btn_iconTXT btn_r btn_m btn_resetOpt" onclick="optionReset()">선택 초기화</button>
						<button class="btn_deepdark btn_r btn_m" onclick="reSearch()">검색하기</button>
					</div>
				</div>
			</div>
		<!-- ******************************************************************************************************************* -->
        ...
        <div class="infoWrap off"> <!-- class="on" -->
            ...
            <div class="infoDetailWrap" style="display: none;">
                ...
                <div class="infoContents">
                    ...
                    <div class="infoDetail">
                        ...
                        <div class="itemvalue">
                            <span><em id="CntPothole">3개</em>포트홀</span>
                            <span><em id="CntVertical">1개</em>수직균열</span>
                            <span><em id="CntHorizontal">0개</em>수평균열</span>
                            <span><em id="CntAlligators">0개</em>피로균열</span>
                        </div>
                        <dl class="">
                            <dt>디바이스명</dt>
                ...
                <div class="btnWrap">
                    <div class="btnArea">
                        <button type="button" class="btn_iconTXT" id="btn_Back"><span class="material-symbols-outlined">list</span> 목록으로</button>
            ...
            <div class="infoListWrap" style="overflow-y:scroll;">
            	...
            	<div class="infoListTop">
                    ...
                    <div class="array-container">
                        <span class="selectBox text bottom" id="select_asc_sort">
                        	<button class="label" id="sort" data-code="desc" onchange="changeSortSelect()">최신순</button>
                            <ul class="optionList" id="sortchk">
                                <li class="optionItem sorting" id="desc" data-code="desc">최신순</li>
                                <li class="optionItem sorting" id="asc" data-code="asc">오래된순</li>
                            </ul>
                        </span>
                    </div>
...
///////////////////// 날짜관련
	...
	old_id = "";
		...
		map = L.map('map').setView({lat:"${authInfo.wtX}", lng:"${authInfo.wtY}"}, 18);
...
function detail(id, device_name, device_id, addr_po_locality, dateFormat, lat, lng, clusterChk){
	...
	for (var i=0; i<allData.length; i++) {
		...
		var popuptxt = "<div><h1>디바이스명 : " + device_name + " ( " + device_id + " )</h1>"
						+ "도로명 : " + addr_po_locality + " (" + lat + ", " + lng + ")<br>"
						+ "촬영일시 : " + dateFormat + "</div>";
		...
		if ( checkboxName == "risk" ) {
			if (checkboxLength == checkedCount) {
				...
				if (checkedCount > 0) {
					...
					$("#alert_msg").html("1개 이상은 필수로 선택되어야 합니다.");
					$("#pop_alert").stop().fadeIn(300);
