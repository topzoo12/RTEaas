...
//@Repository("adtdeviceDAO")
public class AdtDeviceDAO extends AdtAbstractDAO {

	public int insertDevice(FcltsInfoVO paramVO) {
		int iResult = 0;
		try {
			insert("adt_deviceDAO.insertDevice",paramVO);
			iResult = 1;
	...
	public int updateDevice(FcltsInfoVO paramVO) {
		int iResult = 0;
		try {
			insert("adt_deviceDAO.updateDevice",paramVO);
			iResult = 1;
		}catch(NullPointerException e) {
			System.err.println("Null 에러 발생::"+e.toString());
		}
