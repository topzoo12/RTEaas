...
public class MenuDAO extends EgovAbstractDAO {
	...
	public int insertMenu(MenuVO paramVO) {
		int iResult = 0;
		try {
			insert("menuDAO.insertMenu",paramVO);
			iResult = 1;
	...
	public int updateMenu(MenuVO paramVO) {
		int iResult = 0;
		try {
			insert("menuDAO.updateMenu",paramVO);
			iResult = 1;
		}catch(NullPointerException e) {
			System.err.println("Null 에러 발생::"+e.toString());
			iResult = -1;
	...
	public int deleteMenu(MenuVO paramVO) {
		int iResult = 0;
		try {
			insert("menuDAO.deleteMenu",paramVO);
			iResult = 1;
		}catch(NullPointerException e) {
			System.err.println("Null 에러 발생::"+e.toString());
			iResult = -1;
	...
	public void copyMenu(ServiceVO paramVO) {
		update("menuDAO.copyMenu",paramVO);
