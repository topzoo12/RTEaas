...
@Controller
public class UsageLoginController {

	private static final Logger LOGGER = LoggerFactory.getLogger(LoginController.class);

	/** EgovSampleService */
	@Resource(name = "usageLoginService")
	private UsageLoginService usageLoginService;
	...
	@RequestMapping(value = "/sys/usageLoginList.do")
	public ModelAndView usageLoginList(@ModelAttribute("usageSearchVO") UsageSearchVO searchVO, ModelMap model, HttpSession session) throws Exception {
		ModelAndView mv = new ModelAndView("/sys/usageLoginList");
		try {
			...
			List<?> loginList = usageLoginService.selectUsageLoginList(searchVO);

			mv.addObject("resultList", loginList);
			mv.addObject("totCnt", loginList.size());

			List<LoginMenuVO> favList = (List<LoginMenuVO>) authInfo.getFavList();
			for(LoginMenuVO fav : favList){
				if(fav.getSrnUrl().equals("/sys/usageLoginList")) {
	...
	@RequestMapping(value = "/getUsageLoginList.do", method=RequestMethod.POST)
	public void getUsageLoginList(@ModelAttribute("usageSearchVO") UsageSearchVO searchVO, HttpServletResponse response, HttpSession session) throws Exception {
		try {
			...
			jsonObject.put("result", loginList);
