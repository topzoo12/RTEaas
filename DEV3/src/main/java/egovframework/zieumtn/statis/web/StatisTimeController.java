...
@Controller
public class StatisTimeController {

	private static final Logger LOGGER = LoggerFactory.getLogger(StatisTimeController.class);

	/** EgovSampleService */
	@Resource(name = "statisTimeService")
	private StatisTimeService statisTimeService;
	 ...
	 * @exception Exception
	 */
	@RequestMapping(value = "/statis/statisTimeList.do")
	public ModelAndView statisTimeList(@ModelAttribute("statisTimeVO") StatisTimeVO searchVO, ModelMap model, HttpSession session) throws Exception {
		...
		ModelAndView mv = new ModelAndView("/statis/statisTimeList");
		...
		for(LoginMenuVO fav : favList){
			if(fav.getSrnUrl().equals("/statis/statisTimeList")) {
	...
	@GetMapping(value = "/excelStatisTimeList.do")
	public void excelStatisTimeList(@ModelAttribute("statisTimeVO") StatisTimeVO searchVO, HttpServletResponse response, HttpSession session) throws Exception {
		...
		try {
			...
			List<StatisTimeVO> statisList = (List<StatisTimeVO>) statisTimeService.selectStatisTimeList(searchVO);
			...
			if(statisList != null && statisList.size() > 0) {
				...
				// 엑셀 시트명 설정
				sheet = workbook.createSheet("시간별현황");
				...
				//데이터 부분 생성
				for(StatisTimeVO vo : statisList) {
					cellCnt = 0;
					row = sheet.createRow(rowCnt++);
					/*
					 *
	...
	@RequestMapping(value = "/getStatisTimeList.do", method=RequestMethod.POST)
	public void getStatisTimeList(@ModelAttribute("statisTimeVO") StatisTimeVO searchVO, HttpServletResponse response, HttpSession session) throws Exception {
		//ModelAndView mv = new ModelAndView("/getData");
		try {
			AuthVO authInfo = (AuthVO) session.getAttribute("authInfo");
			...
			List<?> statisList = statisTimeService.selectStatisTimeList(searchVO);
