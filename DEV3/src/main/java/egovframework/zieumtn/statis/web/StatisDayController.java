...
@Controller
public class StatisDayController {

	private static final Logger LOGGER = LoggerFactory.getLogger(StatisDayController.class);

	/** EgovSampleService */
	@Resource(name = "statisDayService")
	 ...
	 * @exception Exception
	 */
	@RequestMapping(value = "/statis/statisDayList.do")
	public ModelAndView statisDayList(@ModelAttribute("statisDayVO") StatisDayVO searchVO, ModelMap model, HttpSession session) throws Exception {
		...
		ModelAndView mv = new ModelAndView("/statis/statisDayList");
		...
		for(LoginMenuVO fav : favList){
			if(fav.getSrnUrl().equals("/statis/statisDayList")) {
	...
	@RequestMapping(value = "/getStatisDayList.do", method=RequestMethod.POST)
	public void getStatisDayList(@ModelAttribute("statisDayVO") StatisDayVO searchVO, HttpServletResponse response, HttpSession session) throws Exception {
		...
		try {
			...
			List<?> statisList = statisDayService.selectStatisDayList(searchVO);
	...
	@GetMapping(value = "/excelStatisDayList.do")
	public void excelStatisDayList(@ModelAttribute("statisDayVO") StatisDayVO searchVO, HttpServletResponse response, HttpSession session) throws Exception {
		try {
			AuthVO authInfo = (AuthVO) session.getAttribute("authInfo");
			...
			List<StatisDayVO> statisList = (List<StatisDayVO>) statisDayService.selectStatisDayList(searchVO);
			...
			if(statisList != null && statisList.size() > 0) {
				...
				// 헤더 사이즈
				final int[] colWidths = {
					5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000
				};
				...
				// 엑셀 시트명 설정
				sheet = workbook.createSheet("일별통계");
				...
				for(StatisDayVO vo : statisList) {
					...
					cell = row.createCell(cellCnt++);
					cell.setCellStyle(bodyStyle);
					cell.setCellValue(vo.getDeviceValue22());

					cell = row.createCell(cellCnt++);
					cell.setCellStyle(bodyStyle);
					cell.setCellValue(vo.getDeviceValue23());

					cell = row.createCell(cellCnt++);
					cell.setCellStyle(bodyStyle);
					cell.setCellValue(vo.getDeviceValue24());

					cell = row.createCell(cellCnt++);
					cell.setCellStyle(bodyStyle);
					cell.setCellValue(vo.getDeviceValue25());

					cell = row.createCell(cellCnt++);
					cell.setCellStyle(bodyStyle);
					cell.setCellValue(vo.getDeviceValue26());

					cell = row.createCell(cellCnt++);
					cell.setCellStyle(bodyStyle);
					cell.setCellValue(vo.getDeviceValue27());

					cell = row.createCell(cellCnt++);
					cell.setCellStyle(bodyStyle);
					cell.setCellValue(vo.getDeviceValue28());

					cell = row.createCell(cellCnt++);
					cell.setCellStyle(bodyStyle);
					cell.setCellValue(vo.getDeviceValue29());
					cell = row.createCell(cellCnt++);
					cell.setCellStyle(bodyStyle);
					cell.setCellValue(vo.getDeviceValue30());

					cell = row.createCell(cellCnt++);
					cell.setCellStyle(bodyStyle);
					cell.setCellValue(vo.getDeviceValue31());
				}
