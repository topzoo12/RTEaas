...
@Controller
public class StatisPeriodController {

	private static final Logger LOGGER = LoggerFactory.getLogger(StatisPeriodController.class);

	/** EgovSampleService */
	@Resource(name = "statisPeriodService")
	private StatisPeriodService statisPeriodService;
	 ...
	 * @exception Exception
	 */
	@RequestMapping(value = "/statis/statisPeriodList.do")
	public ModelAndView statisPeriodList(@ModelAttribute("statisPeriodVO") StatisPeriodVO searchVO, ModelMap model, HttpSession session) throws Exception {
		...
		ModelAndView mv = new ModelAndView("/statis/statisPeriodList");
	...
	@RequestMapping(value = "/getStatisPeriodList.do", method=RequestMethod.POST)
	public void getStatisPeriodList(@ModelAttribute("statisPeriodVO") StatisPeriodVO searchVO, HttpServletResponse response, HttpSession session) throws Exception {
		...
		try {
			...
			List<?> statisList = statisPeriodService.selectStatisPeriodList(searchVO);
	...
	@GetMapping(value = "/excelStatisPeriodList.do")
	public void excelStatisPeriodList(@ModelAttribute("statisPeriodVO") StatisPeriodVO searchVO, HttpServletResponse response, HttpSession session) throws Exception {
		//ModelAndView mv = new ModelAndView("/getData");
		try {
			AuthVO authInfo = (AuthVO) session.getAttribute("authInfo");
			...
			List<StatisPeriodVO> statisList = (List<StatisPeriodVO>) statisPeriodService.selectStatisPeriodList(searchVO);
			...
			if(statisList != null && statisList.size() > 0) {
				...
				// 엑셀 시트명 설정
				sheet = workbook.createSheet("기간별통계");
				...
				//데이터 부분 생성
				for(StatisPeriodVO vo : statisList) {
					cellCnt = 0;
					row = sheet.createRow(rowCnt++);
