...
@Controller
public class StatusAirController {

	private static final Logger LOGGER = LoggerFactory.getLogger(StatusAirController.class);

	/** EgovSampleService */
	@Resource(name = "statusAirService")
	private StatusAirService statusAirService;
	 ...
	 * @exception Exception
	 */
	@RequestMapping(value = "/status/statusAirList.do")
	public ModelAndView statusAirList(@ModelAttribute("statusAirVO") StatusAirVO searchVO, ModelMap model, HttpSession session) throws Exception {
		...
		ModelAndView mv = new ModelAndView("/status/statusAirList");
	...
	@RequestMapping(value = "/getStatusAirList.do", method=RequestMethod.POST)
	public void getStatusAirList(@ModelAttribute("statusAirVO") StatusAirVO searchVO, HttpServletResponse response, HttpSession session) throws Exception {
		...
		try {
			...
			List<?> statusList = statusAirService.selectStatusAirList(searchVO);
	...
	@GetMapping(value = "/excelStatusAirList.do")
	public void excelStatusAirList(@ModelAttribute("statusAirVO") StatusAirVO searchVO, HttpServletResponse response, HttpSession session) throws Exception {
		//ModelAndView mv = new ModelAndView("/getData");
		try {
			AuthVO authInfo = (AuthVO) session.getAttribute("authInfo");
			...
			List<StatusAirVO> statusList = (List<StatusAirVO>) statusAirService.selectStatusAirList(searchVO);
			...
			if(statusList != null && statusList.size() > 0) {
				...
				int rowCnt = 0;
				int cellCnt = 0;
				int listCount = statusList.size();

				// 엑셀 시트명 설정
				sheet = workbook.createSheet("대기현황");
