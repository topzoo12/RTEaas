...
public class manageRiskController {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(manageRiskController.class);
	 ...
	 * @exception Exception
	 */
	@RequestMapping(value = "/roadinfo/manageRiskList.do")
	public ModelAndView deviceList(@ModelAttribute("fcltsInfoVO") FcltsInfoVO searchVO, ModelMap model, HttpSession session) throws Exception {
		...
		ModelAndView mv = new ModelAndView("/roadinfo/manageRiskList");

		List<LoginMenuVO> favList = (List<LoginMenuVO>) authInfo.getFavList();
		for(LoginMenuVO fav : favList){
			if(fav.getSrnUrl().equals("/roadinfo/manageRiskList")) {
