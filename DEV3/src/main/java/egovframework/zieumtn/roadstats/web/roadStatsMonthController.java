...
@Controller
public class roadStatsMonthController {

	private static final Logger LOGGER = LoggerFactory.getLogger(roadStatsMonthController.class);
	 ...
	 * @exception Exception
	 */
	@RequestMapping(value = "/roadstats/month.do")
	public ModelAndView deviceList(@ModelAttribute("fcltsInfoVO") FcltsInfoVO searchVO, ModelMap model, HttpSession session) throws Exception {
		...
		System.out.println("******************/roadstats/month");
		...
		ModelAndView mv = new ModelAndView("/roadstats/month");
		...
		for(LoginMenuVO fav : favList){
			if(fav.getSrnUrl().equals("/roadstats/month")) {
